...
static void render_menu_row(unsigned index, int selected)
{
        unsigned y = MENU_Y + index * MENU_LINE_HEIGHT;

        console_clear_line(y);
        console_set_cursor(MENU_X, y);

        console_puts(selected ? "> " : "  ");
        console_puts(menu_items[index]);
}
...
static void render_menu(unsigned selected)
{
        ...
        console_puts("TUXBERRY 0.4-DEV\n");
        console_puts("------------------------------\n\n");

        for (unsigned i = 0; i < MENU_COUNT; i++)
                render_menu_row(i, i == selected);

        console_set_cursor(
                MENU_X,
                MENU_Y + MENU_COUNT * MENU_LINE_HEIGHT + 20u
        );

        console_puts("VOL UP NEXT\n");
...
void menu_run(void *dtb, const struct tb_simplefb *fb, uint32_t machtype)
{
        ...
        for (;;) {
                ...
                if (key == TB_KEY_NEXT) {
                        ...
                        /*
                         * Only redraw the two rows that changed.
                         * This avoids flashing the entire framebuffer.
                         */
                        render_menu_row(old, 0);
                        render_menu_row(selected, 1);
                ...
                /*
                 * Sub-pages clear the framebuffer, so restore the
                 * complete menu when returning from one.
                 */
                render_menu(selected);
        }
}
